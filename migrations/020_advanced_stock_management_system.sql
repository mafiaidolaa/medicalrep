-- ================================================
-- 🏗️ Advanced Stock Management System - Database Schema
-- ================================================
-- Version: 1.0.0
-- Created: 2025-01-10
...
-- Create unique index for reorder recommendations (one per product per warehouse per day)
CREATE UNIQUE INDEX idx_reorder_recommendations_daily 
ON reorder_recommendations(product_id, warehouse_id, DATE(recommended_at));
...
SELECT 
    ...
    SUM(sb.quantity_on_hand) as total_on_hand,
    SUM(sb.quantity_allocated) as total_allocated,
    SUM(sb.quantity_available) as total_available,
    SUM(sb.total_cost) as total_value,
    COUNT(DISTINCT sb.location_id) as locations_count,
    COUNT(DISTINCT sb.batch_number) as batches_count,
    MIN(sb.expiry_date) as earliest_expiry,
    AVG(sb.unit_cost) as average_cost
FROM products p
LEFT JOIN stock_balances sb ON p.id = sb.product_id
LEFT JOIN warehouses w ON sb.warehouse_id = w.id
WHERE (sb.quantity_on_hand > 0 OR sb.quantity_allocated > 0)
GROUP BY p.id, p.name, p.line, w.id, w.name, w.code;
...
SELECT 
    ...
    sm.warehouse_id,
    w.name as warehouse_name,
    DATE_TRUNC('day', sm.movement_date) as movement_date,
    sm.transaction_type,
    sm.reason_code,
    COUNT(*) as movement_count,
    SUM(ABS(sm.quantity_moved)) as total_quantity,
    SUM(sm.total_cost) as total_value
FROM stock_movements sm
JOIN products p ON sm.product_id = p.id
JOIN warehouses w ON sm.warehouse_id = w.id
WHERE sm.movement_date >= CURRENT_DATE - INTERVAL '90 days'
GROUP BY sm.product_id, p.name, sm.warehouse_id, w.name, 
         DATE_TRUNC('day', sm.movement_date), sm.transaction_type, sm.reason_code;
...
SELECT 
    ...
    sa.message,
    sa.created_at,
    EXTRACT(EPOCH FROM (NOW() - sa.created_at))/3600 as hours_old
FROM stock_alerts sa
JOIN products p ON sa.product_id = p.id
JOIN warehouses w ON sa.warehouse_id = w.id
LEFT JOIN warehouse_locations wl ON sa.location_id = wl.id
...
2. 🔄 Update existing product records with stock configurations
3. 📊 Initialize stock balances from current product stock values
4. 🔔 Configure alert thresholds for critical products
5. 📱 Set up mobile app for barcode scanning
6. 🤖 Configure AI models for demand forecasting
7. 📈 Create executive dashboards and reports
8. 🔒 Set up role-based access control
9. 🔄 Schedule periodic analytics calculations
10. 🚀 Performance testing with large datasets

⚡ PERFORMANCE TIPS:
- Partition stock_movements table by date for large datasets
...
🔧 CUSTOMIZATION NOTES:
- Adjust decimal precision based on your business needs
- Modify alert thresholds and rules as required
- Extend barcode formats for your specific requirements
- Add custom fields for industry-specific needs
*/