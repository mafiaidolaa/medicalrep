...
-- إدراج مستويات المخزون التجريبية
INSERT INTO stock_levels (warehouse_id, product_id, available_quantity, shelf_location, bin_location, updated_by) 
SELECT 
    w.id,
    p.id,
    CASE 
        WHEN p.code = 'PRD001' THEN 15
        WHEN p.code = 'PRD002' THEN 25
        WHEN p.code = 'PRD003' THEN 50
        WHEN p.code = 'PRD004' THEN 8
        WHEN p.code = 'PRD005' THEN 3
    END,
    'A-' || (ABS(RANDOM()) % 10 + 1),
    'B-' || (ABS(RANDOM()) % 20 + 1),
    'admin'
FROM warehouses w, products p
WHERE w.code IN ('WH001', 'WH002') AND p.is_active = 1;
