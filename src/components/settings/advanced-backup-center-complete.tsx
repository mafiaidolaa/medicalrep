...
export function AdvancedBackupCenter() {
  ...
  const [selectedComponents, setSelectedComponents] = useState<string[]>(['database', 'user_data', 'settings']);
  ...
  const [selectedBackupLocations, setSelectedBackupLocations] = useState<string[]>(['system_drive']);
  ...
  const cloudServices = [
    ...
    { id: 'mega', name: 'MEGA', icon: Shield, color: 'bg-red-500', freeSpace: '20 GB' },
    { id: 'icloud', name: 'iCloud', icon: Cloud, color: 'bg-gray-600', freeSpace: '5 GB' },
    { id: 'aws_s3', name: 'Amazon S3', icon: Server, color: 'bg-orange-500', freeSpace: 'غير محدود' }
  ];
  ...
  const downloadBackupToDevice = (backupId: string) => {
    ...
    setTimeout(() => {
      ...
      link.href = '#';
      link.download = `backup_${backupId}_${new Date().toISOString().split('T')[0]}.zip`;
      link.click();
  ...
  const stats = { totalBackups: backupHistory.length, successfulBackups: backupHistory.filter(b => b.status === 'نجح').length };
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold bg-gradient-to-r from-green-600 to-emerald-600 bg-clip-text text-transparent">
              النسخ الاحتياطي والاسترداد المتقدم
            </h2>
