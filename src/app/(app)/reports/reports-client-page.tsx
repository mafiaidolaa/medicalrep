        ...
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                    {(() => {
                        // Normalize areas/lines to arrays before mapping
                            ...
                            <>
                                <Select onValueChange={(value) => setSelectedArea(value === 'all' ? null : value)}>
                                    <SelectTrigger className="w-[180px]">
                                        <SelectValue placeholder={t('common.area')} />
                                    </SelectTrigger>
                                    <SelectContent>
                                ...
                                <Select onValueChange={(value) => setSelectedLine(value === 'all' ? null : value)}>
                                    <SelectTrigger className="w-[180px]">
                                        <SelectValue placeholder={t('common.line')} />
                                    </SelectTrigger>
                                    <SelectContent>
                                        <SelectItem value="all">{t('common.all_lines')}</SelectItem>
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                <StatCard title={t('reports.total_revenue')} value={`${totalRevenue.toLocaleString()} ${t('common.egp')}`} icon={DollarSign} />
                <StatCard title={t('reports.total_collections')} value={`${totalCollections.toLocaleString()} ${t('common.egp')}`} icon={HandCoins} />
                <StatCard title={t('reports.total_expenses')} value={`${totalExpenses.toLocaleString()} ${t('common.egp')}`} icon={Receipt} />
                ...
                <CardContent>
                    <ResponsiveContainer width="100%" height={300}>
                        <LineChart data={monthlyRevenueData}>
                            ...
                            <Tooltip formatter={(value: number) => [`${value.toLocaleString()} ${t('common.egp')}`, t('reports.total_revenue')]} />
            ...
            <AgeingDebtReport orders={orders} />
        </div>
    );
}
